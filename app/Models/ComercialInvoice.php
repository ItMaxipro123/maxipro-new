...
class ComercialInvoice extends Model
{
    ...
    public static function getFliterSupplierComercialInvoice($teknisi_cookie,$id)
    {
        try {
            $client = new Client(['verify' => false]);
            ...
            $response = $client->request('GET', 'https://maxipro.id/TeknisiAPI/comercialInvoice_filtersupplier/'.$id, [
                'headers' => $headers,
    ...
    public static function getSelectCategoryComercialInvoiceLclLocal($teknisi_id,$teknisi_cookie,$id,$selected_value)
    {
        try {
            $client = new Client(['verify' => false]);
            ...
            // dd($id,$selected_value,$teknisi_id);
            $response = $client->request('POST', 'https://maxipro.id/TeknisiAPI/comercial_invoice_select_category_lcllocal' , [
                'form_params' => [
                    'id_commercial' =>$id,
                    'selected_value'=>$selected_value,
                    'teknisi' => $teknisi_id,
    ...
    public static function tambahComercialInvoiceNew($teknisi_cookie, $data)
    {
        // dd('aa',$data);
        ...
        try {
            ...
            $formParams = [
                ...
                'discount_nominal'=>$data['discount_nominal'],
                'discount_percent'=>$data['discount_percent'],
                'status_includeppn'=>$data['include_ppn'],
                'freight_cost' => $data['freight_cost'] ?? '',
                'insurance' => $data['insurance'] ?? '',
                'incoterms' => $data['incoterms'] ?? '',
                'location' => $data['location'] ?? '',
            ...
            foreach ($data['status_ppn'] as $key => $value) {
                $lastChar = substr($key, -1);
                $newKey = "status_ppn[{$lastChar}]";
                $newEnglishNames[$newKey] = $value ?? '';
                $key_index++;
                $key_index_array[] = $newKey;
            }
            ...
            $formParams = array_merge($formParams, $newEnglishNames);
            // dd('model',$formParams);
            $response = $client->request('POST', 'https://maxipro.id/TeknisiAPI/comercial_invoice_tambah_new', [
                'form_params' => $formParams,
                'headers' => $headers,
            ]);
